...
// Check connection
    ...
    $sql = "SELECT IMAGEURL,AUTHOR,TITLE,CATEGORY FROM table2 WHERE TITLE LIKE '%$query%' OR AUTHOR LIKE '%$query%'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
    echo "<tr><td><img src='".$row['IMAGEURL']."' width='90' heigth='100'></td><td>" . $row["AUTHOR"]. "</td><td>" . $row["TITLE"] . "</td><td>"
    . $row["CATEGORY"]. "</td></tr>";
    }
    echo "</table>";
    }
